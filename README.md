...
シングルサインオン(SSO)のプロトタイプ実装。

このプロトタイプは、SSOの本番構成を模したシミュレーション構成となっています。  
通常の本番構成では、RPやIdPはそれぞれ別のアプリケーションとなりますが、このプロトタイプでは、１つのアプリケーション内に、RPやIdPを独立したサーブレットとして構築しています。

SSO構成の説明

* `アプリA`：自分のログイン画面を持ち、ユーザー認証（DB照合など）を自前で行う。
* `IdP`：アプリAと同じドメインに属し、Cookieを使って認証状態をシームレスに共有する。アプリAから外部アプリにSSOするために使用する。
* `アプリB`：外部の独立したアプリケーション（サードパーティRP）。
